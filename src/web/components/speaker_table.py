...
def display_speaker_table(speakers: List[Dict[str, Any]]):
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Specializations": st.column_config.TextColumn(
                "Specializations", width="large"
            ),
            "Target Audiences": st.column_config.TextColumn(
                "Target Audiences", width="medium"
            ),
        },
    )
    ...
    for i, speaker in enumerate(speakers, 1):
        with st.expander(
            f"🎤 {speaker.get('name', 'Unknown')} - {speaker.get('job_title', 'Unknown Title')}"
        ):
