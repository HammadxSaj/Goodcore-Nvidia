...
class SpeakerDataProcessor:
    ...
    def _create_single_profile(self, speaker_id: int, group: pd.DataFrame) -> Optional[Dict[str, Any]]:
        ...
        profile = {
            ...
            'active': self._safe_get_boolean(first_row, 'active', True),  # Default to True if unknown
            'bio': str(self._safe_get_value(first_row, 'bio', '')).strip(),
            'specializations': str(self._safe_get_value(first_row, 'specializations', '')).strip(),
            'certifications': str(self._safe_get_value(first_row, 'certifications', '')).strip(),
            'work_experience': str(self._safe_get_value(first_row, 'work_experience', '')).strip(),
            'education': str(self._safe_get_value(first_row, 'education', '')).strip(),
            'industry_specialty': str(self._safe_get_value(first_row, 'industry_specialty', '')).strip(),
        }
